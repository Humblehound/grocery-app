...
function($http, $templateCache, $timeout) {
  ...
  function run() {
    ...
    while (i < 4 && (template = toCache.pop())) {
      // item that inline templates are ignored by this request
...
function($stateProvider, $ionicConfigProvider) {
  ...
  $stateProvider.state = function(stateName, definition) {
    // don't even bother if it's disabled. item, another config may run after this, so it's not a catch-all
...
function($scope, $attrs, $element, $timeout) {
  ...
  // item: this gets processed every scroll event, can it be cached?
